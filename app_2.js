...
async function load() {
  try {
    const [p, b, li, pay, exp] = await Promise.all([
      ...
      supabase.from('bid_line_items').select('*'),
...
async function addProject() {
  ...
  const { data, error } = await supabase.from('projects').insert({ name, address: document.getElementById('pj-addr').value.trim(), city: document.getElementById('pj-city').value.trim(), partners: document.getElementById('pj-partners').value.trim() }).select();
  ...
  ['pj-name', 'pj-addr', 'pj-city', 'pj-partners'].forEach(id => document.getElementById(id).value = '');
...
async function addExpense() {
  ...
  const { data, error } = await supabase.from('expenses').insert({ project_id: pid, description: desc, amount: amt, category: document.getElementById('e-cat').value, paid_by: document.getElementById('e-paidby').value, reimbursed: document.getElementById('e-reimb').value === 'true', status: 'approved' }).select();
...
async function acceptBid(id) {
  ...
  const { error } = await supabase.from('bids').update({ status: 'approved' }).eq('id', id);
...
async function rejectBid(id) {
  ...
  await supabase.from('bids').update({ status: 'rejected' }).eq('id', id);
...
async function revisionBid(id) {
  ...
  if (!note) return;
  await supabase.from('bids').update({ status: 'revision', revision_note: note }).eq('id', id);
...
async function addPayment(bidId, prefix) {
  ...
  const { data, error } = await supabase.from('payments').insert({ bid_id: bidId, note, amount: amt, paid_by: paidBy, paid_at: new Date().toISOString().slice(0, 10) }).select();
