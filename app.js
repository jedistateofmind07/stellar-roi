...
function showTab(t) {
  document.querySelectorAll('.tab').forEach((el, i) => {
    ['dashboard', 'queue', 'bids', 'payments', 'expenses', 'expense-queue', 'reimbursements', 'projects'].forEach((tab, j) => {
...
function renderDash() {
  ...
  document.getElementById('dash-metrics').innerHTML = [
    ...
    { l: 'Approved bid value', v: fmt(approvedVal), c: 'g' },
    { l: 'Pending expenses', v: pendingExp.length, c: pendingExp.length > 0 ? 'a' : '' },
    { l: 'Expense actuals', v: fmt(approvedExpVal), c: 'g' },
    { l: 'Unreimbursed', v: fmt(unreimbursed), c: unreimbursed > 0 ? 'r' : 'g' }
...
function renderExpenseQueue() {
  ...
  if (!el) return;
  if (!pending.length) {
    el.innerHTML = `<div class="card" style="text-align:center;padding:28px"><div style="font-size:22px;color:var(--green);margin-bottom:6px">✓</div><div style="font-size:14px;font-weight:600;color:var(--green)">No pending expenses</div><div style="font-size:12px;color:var(--muted);margin-top:3px">All expenses have been reviewed</div></div>`;
    return;
  }
    ...
    return `<div class="bid-card">
      ...
      <div class="bid-body" id="re-${e.id}">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          <button class="btn sm primary" onclick="approveReimbExpense('${e.id}')">Approve — add to actuals</button>
          <button class="btn sm red" onclick="rejectReimbExpense('${e.id}')">Reject</button>
...
function renderReimbExpenseList() {
  const approved = (C.reimbExpenses || []).filter(e => e.status === 'approved');
  const el = document.getElementById('reimb-approved-list');
  if (!el) return;
  if (!approved.length) { el.innerHTML = `<div style="font-size:13px;color:var(--muted)">No approved expenses yet.</div>`; return; }
  ...
  el.innerHTML = Object.entries(byPerson).map(([person, exps]) => {
    const total = exps.reduce((s, e) => s + (e.total_amount || 0), 0);
    const owed = exps.filter(e => !e.reimbursed).reduce((s, e) => s + (e.total_amount || 0), 0);
    ...
    return `<div class="card" style="margin-bottom:12px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        ...
        <div style="flex:1"><div style="font-size:14px;font-weight:600">${person}</div><div style="font-size:11px;color:var(--muted)">${exps.length} approved expenses — ${fmt(total)} total</div></div>
        <div style="text-align:right"><div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.05em">Outstanding</div><div style="font-size:18px;font-weight:600;color:${owed > 0 ? 'var(--red)' : 'var(--green)'}">${fmt(owed)}</div></div>
      </div>
      ${exps.map(e => {
        const proj = C.projects.find(p => p.id === e.project_id);
        const items = Array.isArray(e.line_items) ? e.line_items : [];
        return `<div style="padding:10px 0;border-top:1px solid var(--border)">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            <div style="flex:1"><div style="font-size:12px;font-weight:500">${items.map(l=>l.desc).filter(Boolean).join(', ') || 'Expense'}</div><div style="font-size:11px;color:var(--muted)">${proj ? proj.name : '—'} &nbsp;·&nbsp; ${e.submitted_at ? e.submitted_at.slice(0,10) : '—'}</div></div>
            <div style="font-size:13px;font-weight:600;color:var(--green)">${fmt(e.total_amount)}</div>
            <button class="btn sm ${e.reimbursed ? 'primary' : ''}" style="font-size:11px;padding:4px 12px;${e.reimbursed ? '' : 'color:var(--amber);border-color:var(--amber)'}" onclick="toggleReimbursed('${e.id}',${!e.reimbursed})">${e.reimbursed ? '✓ Reimbursed' : 'Mark reimbursed'}</button>
          </div>
        </div>`;
      }).join('')}
    </div>`;
...
async function approveReimbExpense(id) {
  showFb('re-fb-' + id, 'saving', 'Approving...', 0);
  const { error } = await db.from('reimbursement_expenses').update({ status: 'approved' }).eq('id', id);
  if (error) { showFb('re-fb-' + id, 'error', 'Error: ' + error.message); return; }
...
async function toggleReimbursed(id, val) {
  const { error } = await db.from('reimbursement_expenses').update({ reimbursed: val, reimbursed_at: val ? new Date().toISOString() : null }).eq('id', id);
  if (!error) {
    const e = C.reimbExpenses.find(x => x.id === id);
