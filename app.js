...
async function refreshBids() {
  const [b, li, pay] = await Promise.all([
    db.from('bids').select('*').order('submitted_at', { ascending: false }),
    db.from('bid_line_items').select('*'),
    db.from('payments').select('*').order('paid_at', { ascending: false })
  ]);
...
function showTab(t) {
  document.querySelectorAll('.tab').forEach((el, i) => {
    ['dashboard', 'queue', 'bids', 'payments', 'expenses', 'expense-queue', 'reimbursements', 'people', 'projects', 'rentals'].forEach((tab, j) => {
  ...
  if (t === 'queue') refreshBids().then(() => renderQueue());
  if (t === 'bids')  refreshBids().then(() => renderBids());
...
function renderQueue() {
  ...
  if (!pending.length) { list.innerHTML = `<div class="card" style="text-align:center;padding:32px"><div style="font-size:22px;color:var(--green);margin-bottom:6px">✓</div><div style="font-size:14px;font-weight:600;color:var(--green)">${t('reimb.queue_clear')}</div><div style="font-size:12px;color:var(--muted);margin-top:3px">No bids waiting for review</div></div>`; return; }
  list.innerHTML = pending.map(b => `<div class="bid-card">
    <div class="bid-hdr" onclick="toggleBody('q-${b.id}')">
      ...
      <span class="bstatus ${b.status === 'revision' ? 'sv' : 'sp'}">${b.status === 'revision' ? 'Revision' : t('status.sp')}</span>
    </div>
...
function renderPaySection(b) {
  ...
  return `<div style="padding-top:12px;border-top:1px solid var(--border)">
    ...
    ${pays.map(p => `<div class="pay-row"><span style="flex:1">${p.note || 'Payment'}</span><span style="color:var(--green)">${fmt(p.amount)}</span><span style="color:var(--muted)">${p.paid_by || '—'}</span><span style="color:var(--muted)">${p.paid_at || '—'}</span><button class="btn sm red" onclick="removePayment('${p.id}','${b.id}','b')">${t('general.remove')}</button></div>`).join('')}
...
function renderPayments() {
  ...
  if (!approved.length) { list.innerHTML = `<div style="font-size:13px;color:var(--muted)">${t('payments.no_data')}</div>`; return; }
  list.innerHTML = approved.map(b => {
    ...
    return `<div class="card">
      ...
      ${pays.map(p => `<div class="pay-row"><span style="flex:1">${p.note || 'Payment'}</span><span style="color:var(--green)">${fmt(p.amount)}</span><span style="color:var(--muted)">${p.paid_by || '—'}</span><span style="color:var(--muted)">${p.paid_at || '—'}</span><button class="btn sm red" onclick="removePayment('${p.id}','${b.id}','p')">${t('general.remove')}</button></div>`).join('')}
  ...
  list.innerHTML = C.projects.map(p => {
    ...
    return `<div style="padding:12px 0;border-bottom:1px solid var(--border);font-size:13px">
      <div style="display:flex;align-items:center;gap:10px;cursor:pointer;margin-bottom:8px" onclick="openProject('${p.id}')">
        ...
        <button class="btn sm red" onclick="event.stopPropagation();removeProject('${p.id}')">${t('general.remove')}</button>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:6px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:5px 10px;font-size:11px;flex:1;min-width:200px;max-width:400px">
          ...
          <button onclick="copyProjLink('${bidLink}','bid-copied-${p.id}')" style="background:var(--green);color:#000;border:none;border-radius:6px;padding:3px 10px;font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;white-space:nowrap">${t('general.copy')}</button>
        ...
        <div style="display:flex;align-items:center;gap:6px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:5px 10px;font-size:11px;flex:1;min-width:200px;max-width:400px">
          ...
          <button onclick="copyProjLink('${expLink}','exp-copied-${p.id}')" style="background:var(--amber);color:#000;border:none;border-radius:6px;padding:3px 10px;font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;white-space:nowrap">${t('general.copy')}</button>
...
function renderPeople() {
  ...
  if (!Object.keys(people).length) {
    el.innerHTML = `<div style="font-size:13px;color:var(--muted)">${t('people.no_people')}</div>`;
    return;
  }
        ...
        ${data.expenses.length ? `<div style="font-size:11px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin:10px 0 6px">Expenses</div>` +
          data.expenses.map(e => {
            ...
            return `<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--border);font-size:12px;flex-wrap:wrap">
              ...
              ${e.status==='approved'?`<span style="color:${e.reimbursed?'var(--green)':'var(--amber)'};">${e.reimbursed?t('general.reimbursed_done')||'✓ Reimbursed':t('people.unpaid')}</span>`:''}
            </div>`;
          }).join('') : ''}
      </div>
    </div>`;
...
function renderExpenseQueue() {
  ...
  if (!el) return;
  if (!pending.length) {
    el.innerHTML = `<div class="card" style="text-align:center;padding:28px"><div style="font-size:22px;color:var(--green);margin-bottom:6px">✓</div><div style="font-size:14px;font-weight:600;color:var(--green)">${t('reimb.queue_clear')}</div><div style="font-size:12px;color:var(--muted);margin-top:3px">${t('reimb.all_reviewed')}</div></div>`;
    return;
  }
...
async function approveReimbExpense(id) {
  ...
  if (error) {
    ...
    if (approveBtn) { approveBtn.disabled=false; approveBtn.textContent=t('queue.approve_actuals'); }
