...
function renderProjects() {
  ...
  list.innerHTML = C.projects.map(p => {
    ...
    return `<div style="display:flex;align-items:center;gap:10px;padding:12px 0;border-bottom:1px solid var(--border);font-size:13px;cursor:pointer" onclick="openProject('${p.id}')">
      <div style="flex:1">
...
function renderReimbExpenseList() {
  ...
  const personHTML = Object.keys(byPerson).length ? Object.entries(byPerson).map(([person, exps]) => {
    ...
    const reimbursed = total - owed;
    const pct = total > 0 ? Math.round((reimbursed/total)*100) : 0;
    ...
    return `<div class="card" style="margin-bottom:12px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:11px;color:var(--muted)">${exps.length} expense${exps.length>1?'s':''} — ${fmt(total)} total paid out</div>
        </div>
        <div style="text-align:right">
      ...
      <div style="height:5px;background:var(--border);border-radius:3px;margin-bottom:12px">
        <div style="height:100%;border-radius:3px;background:${pct>=100?'var(--green)':'var(--amber)'};width:${pct}%"></div>
      </div>
      ${exps.map(e => {
        const proj = C.projects.find(p=>p.id===e.project_id);
        const items = Array.isArray(e.line_items) ? e.line_items : [];
        return `<div style="padding:8px 0;border-top:1px solid var(--border);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <div style="flex:1">
            <div style="font-size:12px;font-weight:500">${items.map(l=>l.desc).filter(Boolean).join(', ')||'Expense'}</div>
            <div style="font-size:11px;color:var(--muted)">${proj?proj.name:'—'} &nbsp;·&nbsp; ${e.submitted_at?e.submitted_at.slice(0,10):'—'}${e.files&&e.files.length?' &nbsp;·&nbsp; 📎 '+e.files.join(', '):''}</div>
          </div>
